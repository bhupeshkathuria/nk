        ...
        if (Mage::getStoreConfig('meigee_categoriesenhanced/options/status')) {
            ...
            foreach ($children as $child) {
				...
				foreach ($_gcategories as $_gcategory) {
					...
					//echo $_gcategoryCount;
				
                if ($childLevel == 1 && ($_gcategoryCount > 0)) {
                ...
                if (!$categoryComplete->getMeigeeCatSubcontent() && ($_gcategoryCount > 0)) {
                    $html .= '<a href="' . $itemUrl . '" ' . $outermostClassCode . '><span' . $subTitle . '>'
                    . $this->escapeHtml($child->getName()) . '</span><span class="top-arrow"></span>' . $catLabel . '</a>';
