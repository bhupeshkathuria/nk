...
class Clay_Salesreport_Block_Margins extends Clay_Salesreport_Block_Abstract{
    ...
    public function getOrders()
    {
		...
		if($post['from'] || $post['to']){
			...
			if($filter){
				...
				$query = "select entity_id,increment_id,grand_total from ".$table." where status = '".$filter."'";
				$result = $connect->query($query);
				$orders = $result->fetchAll($result);
				//$orders = Mage::getResourceModel('sales/order_collection')->addAttributeToSelect('*')->addFieldToFilter('status', array('in' => array($filter)))->addAttributeToSort('entity_id', 'DESC');
			}
			else {
				$connect = Mage::getSingleton('core/resource')->getConnection('core/read');
				$table = Mage::getSingleton('core/resource')->getTableName('sales_flat_order');
				$query = "select entity_id,increment_id,grand_total from ".$table."";
				$result = $connect->query($query);
				$orders = $result->fetchAll($result);
					//$orders = Mage::getResourceModel('sales/order_collection')->addAttributeToSelect('*')->addAttributeToSort('entity_id', 'DESC');
			}
		}
	...
	public function getCustomAttributes($attributeCode, $pID){
		...
		$sql = "SELECT sum(payment_to_supplier) FROM ".$table1." sdi WHERE sdi.sku = '$attributeCode' and sdi.order_id = '$pID'";
