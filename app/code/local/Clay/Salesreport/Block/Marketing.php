...
class Clay_Salesreport_Block_Marketing extends Clay_Courier_Block_Abstract{
    ...
    public function getOrders()
    {
		...
		$sql = "SELECT DATE_FORMAT(o.created_at, '%Y-%m-%d') AS Orderdate,
				 count(o.`increment_id`) AS total_orders,
				 ...
				 SUM(CASE WHEN  o.`status` = 'readypickup' THEN base_amount_ordered ELSE 0 END) AS READYPICKUPTotal,
				 ...
				 SUM(CASE WHEN o.`status` = 'returns' THEN base_amount_ordered ELSE 0 END) AS RETURNTotal,
	...
	public function addMarketSpend($post){
		
		$connect = Mage::getSingleton( 'core/resource' )->getConnection('core_read');
		$table = Mage::getSingleton('core/resource')->getTableName('marketing_spend');
		$sql = "INSERT INTO ".$table."(spend,spend_date) value('".$post['spent_amount']."','".date("Y-m-d",strtotime($post['spent_date']))."')";	
		$result = $connect->query( $sql );
		if($result){
		return 'Done';
		}
