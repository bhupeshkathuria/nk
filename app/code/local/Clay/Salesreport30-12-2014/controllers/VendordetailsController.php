<?php
class Clay_Salesreport_VendordetailsController extends Mage_Core_Controller_Front_Action {
	...
	public function changepaymentstatusAction(){
		echo "---".$paidStatus = $this->getRequest()->getParam('chkpaidstatus');
		echo "---".$paid = $this->getRequest()->getParam('chkpaid');
		echo "---".$supplier = $this->getRequest()->getParam('supplier_name');
		...
		if($paidStatus){
			echo $sql = "UPDATE ".$table1." SET `chkpaidstatus` = '1' where order_number = ".$paidStatus." and supplier_name = '$supplier'";
		}
		else if($paid) {
			$sql = "UPDATE ".$table1." SET `chkpaid` = '1' where order_number = ".$paidStatus." and supplier_name = '$supplier'";
		}
