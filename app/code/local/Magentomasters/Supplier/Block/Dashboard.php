<?php
class Magentomasters_Supplier_Block_Dashboard extends Magentomasters_Supplier_Block_Abstract{
	...
	public function getOrderByStatus($supplierid, $status){
		...
		$sql = "SELECT COUNT(entity_id) AS countOrder,d.price from ".$table." AS d INNER JOIN ".$table2." s ON d.order_id = s.entity_id WHERE d.supplier_id = ".$supplierid." and s.status = '".$status."' GROUP BY s.status";
