...
class Magentomasters_Supplier_Model_Output{
		...
		public function getPdf($order_id,$supplier_id,$items){
			...
			/*require_once 'includes/tcpdf/barcodes.php';
			$barcodeobj = new TCPDFBarcode('http://www.tcpdf.org', 'C128');*/
			$pdf = new TCPDF(PDF_PAGE_ORIENTATION, PDF_UNIT, PDF_PAGE_FORMAT, true, 'UTF-8', false);
			$pdf->setPrintHeader(true);
			$pdf->setPrintFooter(true);
			$order1 = Mage::getModel('sales/order')->load($order_id);
			$shipmentCollection = $order1->getShipmentsCollection();
			 
			 
			/*$pdf->setXY(93,272);
			$bar1 = $pdf->write1DBarcode("165910150581", 'C39', '', '', 90, 10, 0.4, '', 'N');
			$pdf->setBarcode($bar1);*/
			...
			foreach($shipmentCollection as $shipment){
			   ...
			   $bar1 = $pdf->write1DBarcode($track['track_number'], 'C39', '', '', 90, 10, 0.4, '', 'N');
           }
			 }
		...
		private function processTemplate($order,$supplier,$items,$type){
			...
			require_once 'includes/tcpdf/barcodes.php';
			$barcodeobj = new TCPDFBarcode('http://www.tcpdf.org', 'C128');
			$barcodeobj->getBarcodeHTML(2, 30, array(0,0,0));
			...
			$pdf1->setXY(93,272);
			$bar = $pdf1->write1DBarcode("165910150581", 'C39', '', '', 90, 10, 0.4, '', 'N');*/
			
			
			
			$order1 = new Mage_Sales_Model_Order();
			$order_id = $order['increment_id'];
			$order1->loadByIncrementId($order_id);
			$payment_method = $order1->getPayment()->getMethodInstance()->getTitle();
			
			$orderTrack = Mage::getModel('sales/order')->loadByIncrementId($order['increment_id']); 
			$shipmentCollection = $orderTrack->getShipmentsCollection();
			...
			$variables = array(
				...
				'supplier' => $supplier,
				'surname' => $supplier['company'],
				'saddress' => $supplier['address1'],
				'city' => $supplier['city'],
				'postalcode' => $supplier['postalcode'],
				'country' => $supplier['country'],
				'state' => $supplier['state'],
				'tin' => $supplier['custom1'],
				'servicetax' => $supplier['custom2'],
				...
				'total_cost' => Mage::helper("core")->currency($order['grand_total']),
				'sub_total_cost' => Mage::helper("core")->currency($order['subtotal']),
				'discount' => Mage::helper("core")->currency($order['shipping_amount']),
				'barcode' => $barcode,
		...
		private function processitemTemplate($template,$items,$type,$templatevariables,$supplier){	
			...
			foreach($items as $item){
				...
				$variables = array(
					...
					'options' => str_replace("Select a size:","",$customOptionsInfo),
