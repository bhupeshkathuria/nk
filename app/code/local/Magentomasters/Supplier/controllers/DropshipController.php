<?php class Magentomasters_Supplier_DropshipController extends Mage_Adminhtml_Controller_Action
{
	...
	public function dropshipmentAction(){
		...
		$observerModel = Mage::getModel('supplier/observer');
		$postcode = $observerModel->getPostalCode($orderId);
		if($postcode['postcode'] and $postcode['postcode']!=''){
		$isAwbAvailable = $observerModel->getCourierAvailability($postcode['postcode'],$orderId);
		...
		$this->_redirect('adminhtml/sales_order/view/order_id/' . $orderId);	
			...
			Mage::getSingleton('adminhtml/session')->addError("Courier / AWB No. not found" . $e);
			//Mage::getSingleton('adminhtml/session')->setFormData($data);
			$this->_redirect('adminhtml/sales_order/view/order_id/' . $orderId);	
			return;
		}
