<?php
class Magentomasters_Supplier_ReportController extends Mage_Core_Controller_Front_Action {
	...
	public function customstatusAction(){
		$post = $this->getRequest()->getPost();
			...
			for($i = 0; $i < count($post['chkStatus']); $i++){
				
				$cstatus = $post['chkStatus'][$i];	
				$aststus = $post['aStatus'][$i];	
				...
				$query = "UPDATE $table SET status='".$cstatus."' WHERE order_number='".$aorder."'";
				$connect->query($query);
