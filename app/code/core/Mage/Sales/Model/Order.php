...
class Mage_Sales_Model_Order extends Mage_Sales_Model_Abstract
{
    ...
    public function registerCancellation($comment = '', $graceful = true)
    {
        if ($this->canCancel() || $this->isPaymentReview()) {
			...
			if($this->getTotalPaid()){
			$messageM = str_replace(" ","%20","One or more item(s) for your order $orderid has been cancelled. Refund of Rs $totalPaid will be credited to your account within 4-5 business days.");			
			} else {
				$messageM = str_replace(" ","%20","Your order $orderid has been cancelled. Visit netakart.com to view the latest products & exiciting offers.");
			}
			
$url="http://www.netakart.com/sms_code.php?number=$mobile&message=$messageM";
